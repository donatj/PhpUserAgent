...
function parse_user_agent( $u_agent = null ) { 
	...
	if( preg_match('/\((.*?)\)/im', $u_agent, $regs) ) {
		...
		preg_match_all('%(?P<platform>Android|iPhone|iPad|Windows|Linux|Macintosh|Windows Phone OS|Silk|linux-gnu|BlackBerry)(?: i386)?(?: i686)?(?: x86_64)?(?: NT)?(?:[ /][0-9._]+)*(;|$)%im', $regs[1], $result, PREG_PATTERN_ORDER);
